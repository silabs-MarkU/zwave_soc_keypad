...
#if !ISEMPTY(SL_CLI_INST0_COMMAND_GROUP)
...
#endif
static sl_cli_t sl_cli_inst0;
sl_cli_handle_t sl_cli_inst0_handle = &sl_cli_inst0;
sl_cli_command_group_t sl_cli_inst0_command_group =
{
  { NULL },
  false,
...
void sl_cli_instances_init(void)
{
  ...
  // ==== Initialize inst0 ====
  sl_cli_default_handle = sl_cli_inst0_handle;
  sl_cli_default_command_group  = &sl_cli_inst0_command_group;
  instance_parameters.task_name = "CLI instance inst0";
  instance_parameters.iostream_handle = SL_CLI_INST0_IOSTREAM_HANDLE;
#if ISEMPTY(SL_CLI_INST0_COMMAND_GROUP)
  ...
  instance_parameters.stack_size = (SL_CLI_INST0_TASK_STACK_SIZE * sizeof(void *)) & 0xFFFFFFF8u;
  EFM_ASSERT(SL_CLI_INST0_TASK_PRIORITY < osPriorityISR);
  instance_parameters.prio = (osPriority_t)SL_CLI_INST0_TASK_PRIORITY;
  instance_parameters.start_delay_ms = SL_CLI_INST0_TASK_START_DELAY_MS;
  instance_parameters.loop_delay_ms = SL_CLI_INST0_TASK_LOOP_DELAY_MS;
#endif
  sl_cli_instance_init(sl_cli_inst0_handle, &instance_parameters);
...
bool sl_cli_instances_is_ok_to_sleep(void)
{
  ...
  if (sl_cli_is_ok_to_sleep(sl_cli_inst0_handle) == false) {
    return false;
  }
...
sl_iostream_t *previous = sl_iostream_get_default();
  ...
  // Handle inst0
  sl_iostream_set_default(sl_cli_inst0_handle->iostream_handle);
